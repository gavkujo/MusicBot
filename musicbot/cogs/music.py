...
class Music(commands.Cog):
	...
	@commands.command(aliases=["mumbai.chalo", "bhak", "fuck.off"])
	@commands.guild_only()
	...
	@commands.command(aliases=["resume", "p", "shuru.kar", "ruk.ja"])
	@commands.guild_only()
	...
	@commands.command(aliases=["vol", "v", "achhe.se.bol"])
	@commands.guild_only()
	@commands.check(audio_playing)
	@commands.check(in_voice_channel)
	@commands.check(is_audio_requester)
	...
	@commands.command(aliases=["np", "ye.kya.hai", "song.name"])
	@commands.guild_only()
	@commands.check(audio_playing)
	...
	@commands.command(aliases=["cq", "band.kar"])
	@commands.guild_only()
	@commands.check(audio_playing)
	...
	@commands.command(aliases=["madad"])
	@commands.guild_only()
	async def help(self, ctx):
		help_msg = """
**Help message (by yours truly):**
**Meta:**
	``-uptime``     Tells how long the bot has been running.
**Music:**
	``-leave``      Leaves the voice channel, if currently in one.
	``-nowplaying`` Displays information about the current song.
	``-play``       Plays audio from <url>.
	``-queue``      Display the current play queue.
	``-skip``       Skips the currently playing song, or votes to skip it.
  ``-v <number>``     changes volume
**Tips:**
	``-tip``        Get a random tip about using the bot.
​**No Category:**
	``-madad``       Shows this message
Type -madad (मदद) command for more info on a command.
		"""
